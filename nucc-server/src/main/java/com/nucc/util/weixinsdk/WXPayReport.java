                    ...
                    public void run() {
                        while (true) {
                            ...
                            try {
                                ...
                                WXPayUtil.getLogger().info("queryOrder first report msg: {}", firstMsg);
                                String msg = null;
                                sb.append(firstMsg); //会阻塞至有消息
                                int remainNum = config.getReportBatchSize() - 1;
                                for (int j=0; j<remainNum; ++j) {
                                    WXPayUtil.getLogger().info("try queryOrder remain report msg");
                                    // msg = reportMsgQueue.poll();  // 不阻塞了
                                    msg = reportMsgQueue.take();
                                    WXPayUtil.getLogger().info("queryOrder remain report msg: {}", msg);
                                    if (msg == null) {
                                        break;
                                    }
                                    else {
