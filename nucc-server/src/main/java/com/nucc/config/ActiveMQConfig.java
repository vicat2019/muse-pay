...
public class ActiveMQConfig {
    ...
    public RedeliveryPolicy redeliveryPolicy() {
        ...
        redeliveryPolicy.setMaximumRedeliveries(3);         // 重发次数,默认为6次 这里设置为3次
        redeliveryPolicy.setInitialRedeliveryDelay(1);      // 重发时间间隔,默认为1秒
        redeliveryPolicy.setBackOffMultiplier(2);           // 第一次失败后重新发送之前等待500毫秒,第二次失败再等待500 * 2毫秒,这里的2就是value
        redeliveryPolicy.setUseCollisionAvoidance(false);   // 是否避免消息碰撞
        redeliveryPolicy.setMaximumRedeliveryDelay(-1);     // 设置重发最大拖延时间-1 表示没有拖延只有UseExponentialBackOff(true)为true时生效
