...
public class Pusher {
    ...
    public boolean push0x10Message(byte[] uuid) throws Exception {
        ...
        tmp[1] = (byte) appId;
        tmp[2] = (byte) 16;
        System.arraycopy(uuid, 0, tmp, 3, uuid.length);
        tmp[19] = (byte) 0;
        tmp[20] = (byte) 0;
//		out.write(version);
//		out.write(appId);
...
//		out.write(0);
    ...
    public boolean push0x11Message(byte[] uuid, byte[] data) throws Exception {
        ...
        tmp[19] = (byte) 0;
        tmp[20] = (byte) 8;
        System.arraycopy(data, 0, tmp, 21, data.length);
//		out.write(version);
//		out.write(appId);
//		out.write(17);
//		out.write(uuid);
...
//		out.write(uuid);
    ...
    public static void main(String[] args) {
        ...
        try {
            boolean result;
            pusher = new Pusher("localhost", 9999, 5000);
            long start = System.currentTimeMillis();
            for (int i = 500; i < 1000; i++) {
                result = pusher.push0x20Message(StringUtil.md5Byte("" + i), "cmd=ntfurl|title=通知标题|content=通知内容|tt=提示标题|url=/m/admin/eml/inbox/list".getBytes("UTF-8"));
                //result = pusher.push0x10Message(StringUtil.md5Byte(""+i));
                //result = pusher.push0x11Message(StringUtil.md5Byte(""+i),128);
                if (!result) {
